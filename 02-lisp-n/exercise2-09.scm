...
(define (evaluate e env)
  (if (atom? e)
      ...
      (case (car e)
        ...
        ((let)    (let ((variables (map car (cadr e)))
                        (values (map (lambda (d) (cadr d))
                                     (cadr e)))
                        (body (caddr e)))
                    (evaluate (cons `(lambda ,variables ,body) values) env)))
        ((letrec)
         (let ((variables (map car (cadr e)))
               (values (map (lambda (d) (cadr d))
                            (cadr e)))
               (body (cddr e)))
            (let ((new-defs (map (lambda (var) (list var ''void)) variables))
                  (new-body (cons 'begin
                                  (append (map (lambda (var val) `(set! ,var ,val)) variables values)
                                          body))))
              (evaluate (list 'let new-defs new-body) env))))
