...
; This implementation stores an individial stack for each dynamic variable.
; An alternative would be to store just the current values and save old values
; along with the index on the execution stack.
; Both implementations may fail to correctly restore dynamic values in presence
; of continuations/escapes...

(define (meaning-dynamic-let n e e+ r tail?)
  (let ((index (get-dynamic-variable-index n))
        (m (meaning e r #f))
        (m+ (meaning-sequence e+ r #f)))
    (append m (DYNAMIC-PUSH index) m+ (DYNAMIC-POP index))))
...
(define (DYNAMIC-POP index)  (list 241 index))
(define (DYNAMIC-REF index)  (list 240 index))

(define *dynamic-var-values* (make-vector 1024))
...
(define (pop-dynamic-binding index)
  (vector-set! *dynamic-var-values* index (cdr (vector-ref *dynamic-var-values* index))))

(define (push-dynamic-binding index value)
  (vector-set! *dynamic-var-values* index (cons value (vector-ref *dynamic-var-values* index))))
...
(define (find-dynamic-value index)
  (car (vector-ref *dynamic-var-values* index)))
...
(define-instruction (DYNAMIC-POP index) 241
  (pop-dynamic-binding index))
