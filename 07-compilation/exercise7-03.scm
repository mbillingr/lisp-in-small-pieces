...
(definitial 'global-value
  (let* ((arity 1)
    ...
    (make-primitive
      (lambda ()
        (if (= arity+1 (activation-frame-argument-length *val*))
            (let* ((n (activation-frame-argument *val* 0))
                   (i (find-global n 0 sg.current.names)))
              (set! *val* (global-fetch i))
              (set! *pc* (stack-pop)))
            (signal-exception #t (list "Incorrect arity" 'global-value)))))))

(define (find-global n i env)
  (if (pair? env)
      (if (eq? n (car env))
          i
          (find-global n (+ i 1) (cdr env)))
      (signal-exception #t (list "unknown global variable" n))))
