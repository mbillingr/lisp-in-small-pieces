...
(define (invoke f v* s k)
  (if (eq? (f 'type) 'function)
      ((f 'behavior) v* s k)
      (wrong "Not a function" f)))

(define (reify-continuation k)
  (create-function
    k  ; simply tag continuation with itself... this is probably not a good idea :)
    (lambda (v* s kk)
      (k (car v*) s))))

(defprimitive 'call/cc
              (lambda (v* s k)
                (invoke (car v*) (list (reify-continuation k)) s k))
              1)

; convert a target-language list into a host-language list of target-language values
(define (convert-list v* s)
  (if (eq? (v* 'type) 'null)
      '()
      (cons (s (v* 'car))
            (convert-list (s (v* 'cdr)) s))))

(defprimitive 'apply
              (lambda (v* s k)
                (invoke (car v*) (convert-list (cadr v*) s) s k))
              2)
