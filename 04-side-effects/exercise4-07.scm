    ...
    (lambda (a* ss)
      (k (create-function
           ...
           (lambda (v* s k)
             (align-args n* v* s
               (lambda (aligned ss)
                 (let ((n* (map car aligned))
                       (v* (map cdr aligned)))
                   (allocate (length n*) ss
                     (lambda (a* sss)
                       (evaluate-begin e*
                                       (update* r n* a*)
                                       (update* sss a* v*)
...
(define (align-args n* v* s k)
  (cond ((pair? n*)
         (align-args (cdr n*) (cdr v*) s
           (lambda (v ss)
              (k (cons (cons (car n*) (car v*)) v) ss))))
        ((null? n*) (k '() s))
        (else (allocate-list v* s (lambda (vv ss)
                                    (println vv)
                                    (k (list (cons n* vv)) ss))))))
