...
class ReifiedContinuation(Value):
    def __init__(self, k):
        super().__init__()
        self.k = k

    def invoke(self, vs, _r, _k):
        if length(vs) == 1:
            return self.k.resume(vs.car)
        else:
            raise TypeError(
                "Continuations expect one argument {}".format(vs))

definitial(
    'call/cc',
    Primitive(
        'call/cc',
        lambda vs, r, k: vs.car.invoke(cons(ReifiedContinuation(k), Nil), r, k) if length(
            vs) == 1 else wrong(
            TypeError("incorrect arity {} {}".format('call/cc', vs)))))


#   end exercise-specific code
# ========================================================================
