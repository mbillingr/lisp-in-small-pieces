...
; This version of STORE-ARGUMENT puts the values directly in the frame instead
; of temporarily storing them. Note that this implies right-to-left evaluation
; order.

(define (STORE-ARGUMENT m m* rank)
  (lambda ()
    (let ((v* (m*)))
      (println "evaluating value for frame[" rank "]")
      (let ((v (m)))
        (println "frame[" rank "] = " v)
...
(define (ALLOCATE-FRAME size)
  (let ((size+1 (+ size 1)))
    (lambda ()
      (println "allocating frame of size 1 +" size)
