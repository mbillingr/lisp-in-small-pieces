...
(define (meaning e r tail?)
  (if (atom? e)
      ...
      (case (car e)
        ...
        ((redefine)(meaning-redefine (cadr e) r tail?))
        (else      (meaning-application (car e) (cdr e) r tail?)))))


(define (meaning-redefine n r tail?)
  (g.current-redefine! n r)
  (lambda () undefined-value))

(define (g.current-redefine! name r)
  (let ((kind (compute-kind r name)))
    (if kind
        (case (car kind)
          ((predefined)
           (let ((index (g.current-extend! name)))
             (vector-set! sg.current index (vector-ref sg.init (cdr kind)))))
          (else (static-wrong "Wrong redefinition of" (car kind) name)))
        (static-wrong "Unknown variable" name))))
