...
(define (meaning-fix-abstraction n* e+ r tail?)
  (let* ((arity (length n*)))
    (if (= arity 0)
        (ZERO-CLOSURE (meaning-sequence e+ r #t))
        (let ((r2 (r-extend* r n*))
...
(define (meaning-fix-closed-application n* body e* r tail?)
  (let ((arity (length n*)))
    (if (= arity 0)
        (meaning-sequence body r tail?)  ; I guess this is simple inlining
...
(define (ALLOCATE-DOTTED-FRAME arity)
  (let ((arity+1 (+ arity 1)))
    (lambda ()
      (println "allocating dotted frame of size 1 +" arity)
